...
class MotionLoader:
    ...
    def _interpolate_motion(self):
        ...
        # Include every output-grid sample up to the source duration. Using
        # arange(0, duration, dt) dropped the final frame when input and output
        # FPS matched exactly because the endpoint is exclusive.
        self.output_frames = int(np.floor(self.duration / self.output_dt + 1.0e-6)) + 1
        if self.output_frames < 3:
            raise ValueError(
                "The resampled motion must contain at least 3 frames to compute linear and angular velocities."
            )
        times = (
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    while simulation_app.is_running():
        ...
        if reset_flag and not file_saved:
            ...
            # This utility has completed its one-shot conversion. Returning lets
            # main close SimulationApp instead of leaving an idle GPU process.
            return
...
if __name__ == "__main__":
    ...
    main()
    # Isaac Sim can hang inside plugin shutdown after the NPZ has been fully
    # written. Give normal cleanup a chance, then let the OS reclaim resources.
