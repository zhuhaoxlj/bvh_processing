...
- **共享进度条**：按**帧**步进，拖动会同时定位两个窗口的动画，按各自时长的
  同一比例对齐，两边总是停在同一相对位置；右侧读数给出一致的帧号和各自的绝对时间。
  拖动时自动暂停，松手后恢复原来的播放状态。
  快捷键：`空格` 播放/暂停、`←`/`→` 后退/前进一帧（逐帧查看时自动暂停）。
...
- 页面每次请求都从磁盘读取且禁用了缓存，所以**改完页面刷新即可生效**（Python 侧改动仍需
  重启服务）。页头会显示"页面构建 &lt;时间&gt;"，和 `ls -l` 对一下就能确认页面是不是最新的。
...
- `handleOptions`：按顺序执行的整数 JSON 数组。编号含义：`1` 整体去噪、
  `2` 整体平滑（居中滤波，靠近首尾时窗口对称收缩，首尾帧原样保留）、
  `3` 脚步锁定校正、`4` 循环优化、
  `5` 朝向对齐（把人体面向转到 X 轴正方向；朝向由左右大腿根连线判定，
  以第一帧为准、自动跳过开头静止的 T-pose 帧，整段动作绕世界 Y 轴刚性旋转一次，
  动作本身不变形，根位移也随之绕原点旋转）。
